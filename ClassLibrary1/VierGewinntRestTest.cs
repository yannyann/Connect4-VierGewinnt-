...
namespace VierGewinnt.Rest.Test
{
    public class VierGewinntRestTest
    {
        ...
        [Fact]
        public void Should_Return_A_Error_Message_Because_BoardWidth_Is_Not_Set()
        {
            ...
            Assert.Equal("GameException",message.Title);
        }

        [Fact]
        public void Should_Return_A_Error_Message_Because_BoardHeight_Is_Not_Set()
        {
            ...
            var message = JsonConvert.DeserializeObject<DtoMessage>(response.Body.AsString());
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal("The width must be >= 6", message.Message);
            Assert.Equal("GameException", message.Title);
        }
        ...
        [Fact]
        public void Should_Create_A_Session_Game_And_Return_SessionId_Although_PlayerA_Not_Exist_In_DtoSessionStart()
        {
